...
public class Bank {
	...
	public static void main(String[] args) {
		...
		Client client   = null;
		int     menuKey = 0;
		System. out .println(">>> Enter operation client.: 1) Create. 2) Read. 3) Update. 4) Delete. 5) BankDB. 6) Exit");
		if (sc.hasNextInt()) {
			menuKey = sc.nextInt();
		} else {
			sc.next();
		...
		case 4: // Delete client
			...
			if (sc.hasNextInt()) {
				...
				if (status) {
					System.out.println("El cliente " +bank.getClients().get(accNumber).getName() + " se ha borrado correctamente.");
				}
				else {
